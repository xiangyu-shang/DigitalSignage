  ...
  <div class="menu-view">
    ...
    <div class="nav-bar">
      ...
      <div class="delivery-toggle">
        <button class="toggle-btn active">自提</button>
        <button class="toggle-btn">外送</button>
      </div>
      ...
      <div class="search-container">
        ...
        <input type="text" class="search-input" placeholder="羽衣轻体果蔬茶 🍃" />
      </div>
    ...
    <div class="store-info">
      <div class="store-left">
        ...
        <span class="store-name">臣溪工业园店 | 491m</span>
    ...
    <div class="promotion-info">
      ...
      <span class="time-tag">9.9限时礼</span>
    </div>
      ...
      <div class="product-display">
        ...
        <template v-else>
          ...
          <div class="product-list">
            <!-- 分区标题 -->
            <div class="section-title">{{ currentCategory ? currentCategory.name : '当季新品' }}</div>
            ...
            >
              <div class="product-tag" v-if="product.discount > 0.1">新品</div>
              <div 
                class="product-img" 
                :style="{
                ...
                <div class="product-price-row">
                  <div class="price-column">
                    ...
                    <div class="original-price" v-if="product.originalPrice > product.price">
                      <span>面价 ¥{{ product.originalPrice }}</span>
                    </div>
                  </div>
    ...
    <div class="checkout-bar">
      <div class="cart-info" :data-count="cartStore.totalItems">
        <span class="total-price" v-if="cartStore.hasItems">预计到手<span>¥{{ formatPrice(cartStore.totalPrice) }}</span></span>
        <span class="total-price" v-else>购物车为空</span>
        <span class="discount-info" v-if="cartStore.hasItems && calculatedDiscount > 0">已享受更低优惠，共减免¥{{ formatPrice(calculatedDiscount) }}</span>
      </div>
      <button 
        class="checkout-btn" 
        @click="goToCheckout" 
...
onMounted(() => {
  ...
  // 打印诊断信息
  setTimeout(() => {
    console.log('[MenuView] 图片加载状态报告:', getImageLoadReport());
  }, 3000);
});

// 组件卸载前
onBeforeUnmount(() => {
  console.log('[MenuView] 组件卸载前的图片加载报告:', getImageLoadReport());
});
...
function handleImageError(event, product) {
  console.error(`[MenuView] 图片加载失败: ${event.target.src}`, {
  ...
  // 尝试提供更多诊断信息
  const img = new Image();
  img.onload = () => console.log(`[MenuView] 测试加载成功: ${product.thumbnailImage}`);
  img.onerror = () => console.error(`[MenuView] 测试加载也失败: ${product.thumbnailImage}`);
...
.button-divider {
  ...
  height: calc(20 * var(--vw-unit)); /* 调整分割线高度 */
...
.store-info {
  ...
  justify-content: space-between;
  align-items: center;
  padding: calc(16 * var(--vw-unit)); /* 使用vw单位 */
  background-color: white;
  border-bottom: none;
...
.category-item.active {
  ...
  border-left: none;
  position: relative;
  height: calc(66 * var(--vw-unit)); /* 选中时增加高度 */
}
...
.category-item.active .category-logo {
  display: flex;
}
...
@media (max-height: 640px) {
  ...
  .category-item.active {
    height: calc(56 * var(--vw-unit)); /* 较小屏幕上减小选中项高度 */
  }
...
@media (min-height: 800px) {
  ...
  .category-item.active {
    height: calc(72 * var(--vw-unit)); /* 大屏幕上增加选中项高度 */
  }
